...
export class ContainerProductPageComponent implements OnInit, OnDestroy {
  ...
  addProductToCard(event: any) {
    this.subs.add(
      this.basketService
        .addProductToBasket(this.prepareProductObject(event))
        .subscribe((res) => {
          this.notification.success(
            'success',
            'product added successfully to your shopping cart'
          );
          let basketCount = Number(this.basketService.getbasketCount());
          this.basketService.changebasketCount(basketCount + 1);
        })
    );
  }
